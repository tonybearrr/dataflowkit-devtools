<script lang="ts">
	...
	const currentUrl = $derived(`${baseUrl}/${lang}/json`);
...
<div class="max-w-7xl mx-auto px-4 py-8">
	...
	{#if isLarge}
		...
		>
			<p class="text-sm text-[var(--color-warning)]">
				{tStringReactive('json.largeJsonWarning', $locale)}
			</p>
		</div>
	{/if}
	...
	<div class="grid lg:grid-cols-2 gap-8">
		<div class="space-y-4">
			...
			>
				...
				<div class="flex flex-wrap items-center gap-4 mt-3 text-xs">
					<label class="flex items-center gap-2 cursor-pointer">
						...
						/>
						<span class="text-[var(--color-text-muted)]">{tStringReactive('json.autoValidate', $locale)}</span>
					</label>
					<label class="flex items-center gap-2 cursor-pointer">
						<input
							type="checkbox"
							...
							class="rounded border-[var(--color-border)] bg-[var(--color-bg-tertiary)]"
						/>
						<span class="text-[var(--color-text-muted)]">{tStringReactive('json.sortKeys', $locale)}</span>
					</label>
				</div>
			</div>
			...
			{#if parseResult}
				{#if parseResult.ok}
					...
					>
						<p class="text-sm text-[var(--color-success)]">{tStringReactive('json.validJson', $locale)}</p>
					</div>
				{:else}
					<div
						class="p-3 rounded-lg border border-[var(--color-error)]/30 bg-[var(--color-error)]/10"
					>
						<p class="text-sm text-[var(--color-error)]">{tStringReactive('json.invalidJson', $locale)}</p>
						<p class="text-xs text-[var(--color-text-muted)] mt-1">{parseResult.error}</p>
						{#if parseResult.line || parseResult.column}
							<p class="text-xs text-[var(--color-text-muted)]">
								{tStringReactive('json.line', $locale)} {parseResult.line}, {tStringReactive('json.column', $locale)} {parseResult.column}
							</p>
						...
						{#if bracketMessage}
							<div class="mt-3 p-2 rounded bg-[var(--color-warning)]/10 border border-[var(--color-warning)]/30">
								<p class="text-xs text-[var(--color-warning)] font-medium mb-1">{tStringReactive('json.tip', $locale)}</p>
		...
		<div class="space-y-4">
			...
			>
				...
				>
					{tStringReactive('json.output', $locale)}
				</button>
				<button
					...
					class="flex-1 px-3 py-2 text-sm rounded-md transition-colors {activeTab === 'tree'
						...
						: 'text-[var(--color-text-muted)] hover:text-[var(--color-text)]'}"
				>
					{tStringReactive('json.tree', $locale)}
				</button>
			</div>
				...
				>
						...
						{#if parsedValue !== null}
							<div class="flex gap-2">
								<button
									...
									class="text-xs px-2 py-1 rounded border border-[var(--color-border)] hover:border-[var(--color-accent)] transition-colors"
								>
									{tStringReactive('json.copyOutput', $locale)}
								</button>
								<button
									onclick={copyMinified}
									class="text-xs px-2 py-1 rounded border border-[var(--color-border)] hover:border-[var(--color-accent)] transition-colors"
								>
									{tStringReactive('json.copyMinified', $locale)}
								</button>
							</div>
						{/if}
					</div>
					...
					{:else}
						<div class="py-8 text-center text-[var(--color-text-muted)]">
							<p>{tStringReactive('json.enterValidJson', $locale)}</p>
						</div>
					{/if}
				</div>
			{:else}
				...
				>
					<div class="flex items-center justify-between mb-3">
						...
						>
							{tStringReactive('json.tree', $locale).toUpperCase()}
						</span>
					</div>
					...
					<div class="mb-3">
						<input
							...
							placeholder={tStringReactive('json.searchPlaceholder', $locale)}
	...
	<section class="mt-16 prose prose-invert max-w-none">
		<h2 class="text-xl font-semibold mb-4">{tStringReactive('json.about', $locale)}</h2>
		<p class="text-[var(--color-text-muted)] mb-4">
			{tStringReactive('json.aboutDescription', $locale)}
		</p>

		<h3 class="text-lg font-medium mt-6 mb-3">{tStringReactive('json.features', $locale)}</h3>
		<ul class="text-[var(--color-text-muted)] space-y-2 list-disc list-inside">
			<li><strong>{tStringReactive('json.prettify', $locale)}</strong> — {tStringReactive('json.featurePrettify', $locale)}</li>
			<li><strong>{tStringReactive('json.minify', $locale)}</strong> — {tStringReactive('json.featureMinify', $locale)}</li>
			<li><strong>{tStringReactive('json.validate', $locale)}</strong> — {tStringReactive('json.featureValidate', $locale)}</li>
			<li><strong>{tStringReactive('json.tree', $locale)}</strong> — {tStringReactive('json.featureTreeView', $locale)}</li>
			<li><strong>{tStringReactive('json.search', $locale)}</strong> — {tStringReactive('json.featureSearch', $locale)}</li>
			<li><strong>{tStringReactive('json.sortKeys', $locale)}</strong> — {tStringReactive('json.featureSortKeys', $locale)}</li>
		</ul>

		<h3 class="text-lg font-medium mt-6 mb-3">{tStringReactive('json.tips', $locale)}</h3>
		<ul class="text-[var(--color-text-muted)] space-y-2 list-disc list-inside">
			<li>{tStringReactive('json.tip1', $locale)}</li>
			<li>{tStringReactive('json.tip2', $locale)}</li>
			<li>{tStringReactive('json.tip3', $locale)}</li>
		</ul>

		<h3 class="text-lg font-medium mt-6 mb-3">{tStringReactive('json.privacyNote', $locale)}</h3>
		<p class="text-[var(--color-text-muted)]">
			{tStringReactive('json.privacyText', $locale)}
		</p>
	</section>
</div>
